...
- U1のピン直近など、電流経路上の短い引き出し配線(1mm前後)は0.4〜0.6mmへテーパーさせている箇所があります(実配線上、U1の隣接ピン間クリアランスを確保するための現実的な折衷で、区間長がごく短いため発熱・電圧降下への影響は無視できます)。
- `J1 5V → F1`区間(`VBUS_5V`ネット、約31mm)は基板既定の0.2mmのまま据え置いています。上記IPC-2221計算のとおり0.2mmでも1oz・ΔT10℃で約0.74Aまで許容でき、ILIM実力値の上限(562.4 mA)を上回るため電気的な問題はありませんが、J1の23pinヘッダー直近を通る密な既存配線を今回のタスク範囲外で引き直すリスクを避け、拡幅は見送りました。今回新設・再配線した`F1→U1→SW4→COUT/J2`区間はすべて1.0mmです。

## GND強化

B.Cu全面をGNDゾーン化してリフィルしました。既存のGNDビアは1本のみでしたが、U1/CIN/R_ILIM周辺への追加ステッチビアにより、現在GNDネットのビアは4本(F.Cu-B.Cu貫通)です。ゾーンリフィル後に`kicad-cli pcb drc --refill-zones --save-board`でDRCを再実行し、0違反であることを確認しています。
