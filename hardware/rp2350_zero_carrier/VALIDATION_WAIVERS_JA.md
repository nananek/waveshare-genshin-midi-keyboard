...
| WVR-004 | 要メーカー確認 | JLC実装プレビュー | native-position generator/`--check`、BOM 5材料/CPL 6搭載点 contract PASS、R4/R5=`C14675` grouped BOM、公式orientation guide | 新規JLC sessionでC14675=`R4,R5`/Qty=2（またはR4=1/R5=1）かつwarning/Qty=0/未選択なし、U1 pin1と6点のrotationを画像保存 |
...
| WVR-008 | 要メーカー確認 | 6点の在庫・代替・実装方向 | 発注判断書のexact Manufacturer/MPN/LCSC 6行表、BOM LCSC固定 | 注文時に全代替を禁止し、U1=`TPS2553DBVR`（`-1`禁止）を含む6点のMPN/値/誘電体/許容差とC14675の調達可否を確認 |
